            ...
            <div className={`w-full max-w-5xl transition-all duration-1000 ${isRevealed ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-10 scale-95'}`}>
                ...
                <div className="text-center mb-16">
                    <div className="flex justify-center mb-8">
                        <div className="w-20 h-20 glass-morphism rounded-3xl flex items-center justify-center border-white/20 shadow-2xl overflow-hidden cursor-pointer hover:scale-110 transition-all duration-500 relative z-10" onClick={() => navigate('/')}>
                            {!logoError ? (
                                <img src={logo} alt="Electra" className="w-full h-full object-contain p-4 transition-transform duration-700" onError={() => setLogoError(true)} />
                            ) : (
                    ...
                    <div className="space-y-4">
                        <p className="text-[12px] font-black uppercase tracking-[0.8em] text-cyan-400 mb-2 italic">Phase 02: Settlement</p>
                        <h2 className="font-accent text-5xl md:text-6xl italic tracking-tighter uppercase mb-6 leading-none">SECURE <span className="text-primary text-glow-cyan">VAULT</span></h2>
                    </div>
                </div>
                ...
                <div className="grid lg:grid-cols-2 gap-10 items-stretch">
                    ...
                    <div className="glass-morphism p-10 flex flex-col justify-between border-t-8 border-t-lime-500 rounded-[2.5rem]">
                        <div className="space-y-8">
                            <h4 className="font-accent text-3xl italic tracking-tighter uppercase leading-none text-white/90">TRANSMIT <span className="text-lime-400">RECEIPT</span></h4>

                            <div className="relative group min-h-[250px] flex">
                                ...
                                <div className={`flex-1 border-2 border-dashed rounded-[2rem] flex flex-col items-center justify-center p-8 transition-all ${screenshot ? 'border-lime-500 bg-lime-500/5' : 'border-white/10 group-hover:border-white/30 bg-white/5'}`}>
                                    {screenshot ? (
                                        <div className="text-center space-y-4 animate-fade-in-up">
                                            <div className="text-5xl text-lime-400">✔</div>
                                            <p className="text-[10px] font-black uppercase tracking-widest text-lime-400 truncate max-w-[200px]">{screenshot.name}</p>
                                        </div>
                                    ) : (
                                        <div className="text-center space-y-6">
                                            <div className="text-6xl opacity-10">📸</div>
                                            <p className="text-[10px] font-black uppercase tracking-[0.5em] text-white/20">Upload Screenshot</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                        ...
                        <button
                            ...
                            className={`w-full py-8 btn-premium text-2xl transition-all ${!screenshot ? 'opacity-20 grayscale cursor-not-allowed' : 'hover:scale-[1.02]'}`}
                        >
