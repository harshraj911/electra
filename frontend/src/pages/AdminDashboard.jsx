    ...
    return (
        <div className="min-h-screen bg-[#020617] text-white p-6 md:p-12 font-sans relative">
            <div className="fixed inset-0 pointer-events-none z-0">
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-[1000px] h-[1000px] bg-primary/5 blur-[250px] -z-10 animate-pulse"></div>
            </div>
            ...
            <div className="max-w-7xl mx-auto space-y-12">
                <header className="flex flex-col md:flex-row justify-between items-center gap-8 animate-fade-in-up">
                    ...
                    <div className="flex flex-wrap justify-center gap-4">
                        <button onClick={handleDownload} className="px-6 sm:px-8 py-3 glass-morphism font-black uppercase text-[10px] tracking-widest hover:bg-white hover:text-black transition-all">Export XLSX</button>
                        <button onClick={handleClearData} className="px-6 sm:px-8 py-3 bg-red-600/10 border border-red-500/50 text-red-500 font-black uppercase text-[10px] tracking-widest hover:bg-red-600 hover:text-white transition-all">Clear All</button>
                        <button onClick={() => { localStorage.removeItem('isAdmin'); navigate('/admin/login'); }} className="px-6 sm:px-8 py-3 bg-white/5 border border-white/10 font-black uppercase text-[10px] tracking-widest hover:bg-white/10">Exit</button>
                    </div>
                </header>

                <div className="grid lg:grid-cols-4 gap-12">
                    {isMobile ? (
                        <div className="lg:col-span-4 flex flex-col items-center justify-center py-24 glass-morphism rounded-[3rem] border-t-8 border-t-red-500 animate-pulse text-center space-y-8">
                            <div className="text-8xl">🚫</div>
                            <h2 className="font-accent text-4xl italic text-red-500">RESTRICTED TERMINAL</h2>
                            <p className="max-w-md text-[12px] font-black uppercase tracking-[0.5em] text-white/40 leading-relaxed italic">
                                THIS DATA STREAM IS ONLY ACCESSIBLE VIA <br />
                                HIGH-CAPACITY DESKTOP INTERFACES. <br />
                                MOBILE ACCESS IS PROHIBITED FOR SECURITY.
                            </p>
                            <button onClick={() => navigate('/')} className="btn-premium px-12 py-5 text-sm">EXIT TO BASE</button>
                        </div>
                    ) : (
                        <>
                            {/* Main List */}
                            <div className="lg:col-span-3 space-y-8">
                                <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
                                    <input type="text" placeholder="FILTER ROSTER..." className="w-full sm:flex-grow bg-white/5 border border-white/10 p-4 outline-none focus:border-primary font-black uppercase text-xs tracking-widest" value={search} onChange={(e) => setSearch(e.target.value)} />
                                    <select className="w-full sm:w-48 bg-white/5 border border-white/10 p-4 outline-none font-black text-xs uppercase tracking-widest" value={gameFilter} onChange={(e) => setGameFilter(e.target.value)}>
                                ...
                                <div className="glass-morphism overflow-x-auto custom-scrollbar">
                                            ...
                                            {filteredWaitlist.map((reg, idx) => (
                                                <tr key={idx} className="hover:bg-white/5 transition-all group">
                                                    ...
                                                    <td className="p-6 space-y-4 align-top">
                                                        ...
                                                        <div className="bg-white/5 px-4 py-2 border border-white/10 inline-block">
                                                            <p className="text-[8px] font-black uppercase text-white/20 tracking-widest mb-1">Unique Protocol ID</p>
                                                            <p className="font-bold text-xs text-primary italic select-all select-none">{reg.unique_id}</p>
                                                        </div>
                                                        <br />
                                                        {reg.ss_url ? (
